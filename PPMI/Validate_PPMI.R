...
# Make prediction
load("~/Data/Fit_EMIF_MCI_EN.RData")
pred_EN <- predict(fit, predictedScore_factors_fil, type = "prob")
load("~/Data/Fit_EMIF_MCI_sPLS.RData")
...
boxplot(pred_EN$MCI ~ as.character(metaData_fil$Class))
...
testDF <- cbind.data.frame(predictedScore_factors_fil,
                           ...
                           pred_RF = pred_RF$MCI,
                           metaData_fil)
model <- lm(Class ~ age  + pred_RF, data = testDF)
summary(model)

model <- glm(Class ~ age + pred_RF, data = testDF,family = binomial)
summary(model)
...
# AUC
score <- c("pred_EN", "pred_sPLS","pred_RF", "EpiAge")
scoreName <- c("Epi-MCI (EN)","Epi-MCI (sPLS-DA)","Epi-MCI (RF)","Epi-Age")
...
ggsave(p, file = "PPMI/ROC_CI_PPMI_v2.png", width = 8, height = 5)
