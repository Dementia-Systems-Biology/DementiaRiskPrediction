...
samples <- intersect(samples_apoe, samples_csf)
...
colnames(X_train) <- c(colnames(X_train)[-18], "APOE")
colnames(X_test) <- c(colnames(X_test)[-18], "APOE")
...
# Save model
save(fit, file = "EMIF/Fit_EMIF_MCI_EN_CSF_APOE.RData")
...
# Actual training
set.seed(123)
fit <- train(x = X_train[,c("Ptau_ASSAY_Zscore", "Ttau_ASSAY_Zscore", "AB_Zscore", "APOE")],
             y = Y_train$Y,
             metric= performance_metric,
             method = MLmethod,
             tuneGrid = parameterGrid,
...
# Save model
save(fit, file = "EMIF/Fit_EMIF_MCI_EN_CSF_APOEonly.RData")
...
# Save model
save(fit, file = "EMIF/Fit_EMIF_MCI_sPLS_CSF_APOE.RData")
...
# Save model
save(fit, file = "EMIF/Fit_EMIF_MCI_sPLS_CSF_APOEonly.RData")
...
# Save model
save(fit, file = "EMIF/Fit_EMIF_MCI_RF_CSF_APOE.RData")
...
X_train1 <- X_train[,c("Ptau_ASSAY_Zscore", "Ttau_ASSAY_Zscore", "AB_Zscore", "APOE")]
...
# Save model
save(fit, file = "EMIF/Fit_EMIF_MCI_RF_CSF_APOEonly.RData")
...
load("EMIF/Fit_EMIF_MCI_RF_CSF_APOE.RData")
RF <- predict(fit, X_test, type = "prob")

load("EMIF/Fit_EMIF_MCI_EN_CSF_APOE.RData")
EN <- predict(fit, X_test, type = "prob")

load("EMIF/Fit_EMIF_MCI_sPLS_CSF_APOE.RData")
sPLS <- predict(fit, X_test, type = "prob")

load("EMIF/Fit_EMIF_MCI_RF_CSF_APOEonly.RData")
RFonly <- predict(fit, X_test, type = "prob")

load("EMIF/Fit_EMIF_MCI_EN_CSF_APOEonly.RData")
ENonly <- predict(fit, X_test, type = "prob")

load("EMIF/Fit_EMIF_MCI_sPLS_CSF_APOEonly.RData")
...
scoreName1 <- c("APOE/CSF (EN)\t\t\t",
               "APOE/CSF (sPLS-DA)\t\t","APOE/CSF (RF-RFE)\t\t",
               "MRSs/APOE/CSF (EN):\t\t", "MRSs/APOE/CSF (sPLS-DA):", 
               "MRSs/APOE/CSF (RF-RFE):")
...
ggsave(p, file = "EMIF/ROC_MCI_EMIF_CSF_APOE.png", width = 7, height = 5)
