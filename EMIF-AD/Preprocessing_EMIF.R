...
# Recalculate bisulfite conversion
bsc <- data.frame(bsc = wateRmelon::bscon(RGset_all))
...
# Save plot
ggsave(bisulfiteConv, file = paste0(OutputDir,"bisulfiteConv_EMIF.png"), height = 6, width = 8)
...
ggsave(bisulfiteConv, file = paste0(OutputDir,"bisulfiteConv_EMIF1.png"), height = 6, width = 8)
...
# Recalculate median intensities
qc <- getQC(preprocessRaw(RGset_all))
plotQC <- data.frame(ID = qc@rownames,
                     mMed = qc$mMed,
                     uMed = qc$uMed)
...
# Save plot
ggsave(p, file = paste0(OutputDir,"sampleQuality_EMIF.png"), width = 8, height = 6)
...
# Save plot
ggsave(p, file = paste0(OutputDir,"sampleQuality_EMIF1.png"), width = 8, height = 6)
...
predictedSex <- inner_join(predictedSex,metaData_EMIF, by = c("SampleID" = "X"))
predictedSex$Sex <- ifelse(predictedSex$Gender == 0, "M", "F")

keep <- predictedSex$SampleID[predictedSex$PredictedSex == predictedSex$Sex]
RGset_all <- RGset_all[,keep]

# Make plot
matchingSex <- ggplot(predictedSex[predictedSex$SampleID %in% keep,]) +
...
# Save plot
ggsave(matchingSex, file = paste0(OutputDir,"matchingSex_EMIF.png"), width = 8, height = 6)


# Predicted sex
predictedSex <- getSex(mapToGenome(RGset_all1), cutoff = -2)
predictedSex <- data.frame(
  SampleID = predictedSex@rownames,
  PredictedSex = predictedSex$predictedSex,
  xMed = predictedSex$xMed,
...
# Save plot
ggsave(matchingSex, file = paste0(OutputDir,"matchingSex_EMIF1.png"), width = 8, height = 6)
...
# Combine data
all(rownames(methSet_allNorm) == rownames(methSet_allNorm1))
X_EMIF <- cbind(methSet_allNorm, methSet_allNorm1)
...
lumi_dpval <- lumi_dpval[rownames(X_EMIF), colnames(X_EMIF)]
save(lumi_dpval, file = paste0(DataDir,"lumi_dpval_EMIF.RData"))

#PCA
pcaList <-  prcomp(t(X_EMIF[rowSums(lumi_dpval > 0.01) == 0,]),        
                   retx = TRUE,
                   center =TRUE,
                   scale = TRUE,
                   rank. = 10)
